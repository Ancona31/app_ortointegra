...
export default function ConsentimientoInformadoForm({
  ...
  borradorId: borradorIdInicial, offlineMode, onOfflineSave, onVacioChange, onCerrarTrasEmitir,
  ...
  async function sellar(firmas: FirmaCapturada[], previstos: number): Promise<void> {
    ...
    try {
      ...
      setSellando(false)
      if (pdfBlob) { setDocGenerado({ blob: pdfBlob, guardado: true, documentoId: firmando?.documentoId ?? null }); setHuellaEmitida(huella) }
    }
  }
