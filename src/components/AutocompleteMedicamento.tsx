...
export default function AutocompleteMedicamento({ value, onChange, onSelect, placeholder = 'Ej: VOLTAREN', className = '' }: Props) {
  ...
  // Búsqueda 100% local sobre MEDICAMENTOS bundled — sin red, instantáneo
  const resultados: MedicamentoDB[] = useMemo(() => {
  ...
  const showDropdown = open && shouldSearch && resultados.length > 0
