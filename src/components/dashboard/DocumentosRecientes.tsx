...
export default function DocumentosRecientes() {
  ...
  if (error) return (
    <div>
      {encabezado}
      <div className="mt-[var(--sp-gap-tiles)]"><BotonNuevo /></div>
          ...
          return (
            ...
            >
              ...
              <span className="flex items-center gap-[var(--sp-1-5)] min-w-0">
                ...
                <span
                  className="shrink-0 text-[length:var(--sp-fs-legal)] font-bold uppercase tracking-[0.04em]"
                  style={meta ? { color: `var(--sp-doc-${meta.token})` } : { color: 'var(--sp-ink-350)' }}
                >
                  {meta?.label ?? 'Documento'}
                </span>
                <span className="ml-auto shrink-0 text-[length:var(--sp-fs-legal)] text-[var(--sp-ink-350)]">
                  {formatDistanceToNow(parseISO(doc.created_at), { locale: es, addSuffix: true })}
                </span>
              </span>
              <span className="truncate text-[length:var(--sp-fs-body-sm)] font-semibold text-[var(--sp-ink-800)]">
