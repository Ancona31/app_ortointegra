...
export default function ModalVisorDocumento({ doc, onClose, pacienteEmail }: Props) {
  ...
  return (
    <ModalShell
      open={true}
      onClose={onClose}
      title={TIPO_DOC_LABEL[doc.tipo] || doc.tipo}
      subtitle={doc.created_at ? format(parseISO(doc.created_at), "dd 'de' MMMM 'de' yyyy", { locale: es }) : undefined}
      maxWidth="max-w-lg"
      footer={footer}
    >
      <div className="px-5 py-4 space-y-4 text-sm text-slate-700">
        {/* Badge colorido de tipo como primera fila del body */}
        <div>
          <span className={`inline-block text-xs px-2.5 py-1 rounded-full font-medium ${TIPO_DOC_COLOR[doc.tipo] || 'bg-slate-100 text-slate-600'}`}>
