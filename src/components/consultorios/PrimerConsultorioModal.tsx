...
const ZONAS_MEXICO: { value: string; label: string }[] = [
  { value: 'America/Mexico_City',    label: 'Centro — CDMX, Edomex, mayoría' },
  { value: 'America/Cancun',          label: 'Quintana Roo — Cancún' },
  { value: 'America/Merida',          label: 'Yucatán — Mérida' },
  { value: 'America/Monterrey',       label: 'Nuevo León, Tamaulipas' },
  { value: 'America/Matamoros',       label: 'Tamaulipas — frontera norte' },
  { value: 'America/Mazatlan',        label: 'Sinaloa, Nayarit, BCS' },
  { value: 'America/Chihuahua',       label: 'Chihuahua' },
  { value: 'America/Ojinaga',         label: 'Chihuahua — Ojinaga' },
  { value: 'America/Hermosillo',      label: 'Sonora — sin DST' },
  { value: 'America/Bahia_Banderas',  label: 'Nayarit — Punta Mita' },
  { value: 'America/Tijuana',         label: 'Baja California — con DST' },
  { value: 'America/Ciudad_Juarez',   label: 'Chihuahua — Cd. Juárez' },
]

const CHIPS_RAPIDOS: { value: string; label: string }[] = [
  { value: 'America/Mexico_City', label: 'Centro (CDMX, mayoría del país)' },
  { value: 'America/Cancun',      label: 'Quintana Roo (Cancún)' },
        ...
        <div>
          ...
          <div className="flex flex-wrap gap-2 mb-2">
            {CHIPS_RAPIDOS.map((chip) => {
              ...
              const esDetectado = zonaNavegador === chip.value
              return (
                <button
                  key={chip.value}
                  type="button"
                  ...
                  className={`px-3 py-1.5 text-xs rounded-full border transition-colors ${
                    seleccionado
                      ...
                      : esDetectado
                        ? 'bg-blue-50 text-[var(--cp)] border-[var(--cp)]/50'
          ...
          <input
            ...
            placeholder="Otra zona — empieza a escribir para buscar"
            className="w-full px-3 py-2 text-sm border border-slate-200 rounded-xl focus:outline-none focus:border-[var(--cp)] transition-colors"
          />
          <datalist id="zonas-mexico">
            {ZONAS_MEXICO.map((z) => (
          ...
          {zonaNavegador && (
            <p className="text-[11px] text-slate-500 mt-1">
              Zona detectada del navegador: <span className="font-mono">{zonaNavegador}</span>
            </p>
          )}
        </div>
      </div>
