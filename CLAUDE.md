...
4. **`TabGraficas.tsx` vive temporalmente en disco como archivo utilitario.** `normalizarKey` y `ParamGrafica` siguen siendo importados por `useLaboratoriosNormalizados`. Se eliminará completo cuando reestructuremos el sistema de laboratorios (próximo chat dedicado). No renderizado en ninguna página.