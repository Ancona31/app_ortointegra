...
| # | Nombre | Tipo | Tamaño | Bloquea | Depende |
|---|---|---|---|---|---|
| 0 | Cleanup y deuda técnica documentada | chore | XS | — | — |
| 1A | Schema SQL: tablas + RLS + trigger | infra | M | 1B, 2 | 0 |
...
| 8 | Migración /estado al nuevo modelo + drop de tabla legacy | refactor + SQL | L | 9 | 4, 5 |
...
### SUB-FASE 8 — Migración /estado al nuevo modelo + drop tabla legacy

**Objetivo**: Migrar el dashboard `/expediente/[id]/estado` para que consuma `mediciones_analitos` en lugar de la tabla `laboratorios` legacy + `useLaboratoriosNormalizados`. Después, eliminar legacy.

**Scope atómico**:
- Auditar consumidores actuales de `useLaboratoriosNormalizados` en `/estado` — listar componentes y queries
- Crear `src/hooks/useUltimasMedicionesParaDashboard(pacienteId)`: equivalente al hook viejo pero leyendo del nuevo modelo
- Refactorizar componentes de `/estado` para consumir el nuevo hook
- Verificar que la UI de `/estado` se ve igual o mejor (sin regresiones visuales)
- Eliminar `src/hooks/useLaboratoriosNormalizados.ts` (ahora huérfano)
- Eliminar página vieja `/expediente/[id]/laboratorios/page.tsx` y subrutas existentes (si no se hizo en sub-fase 0)
- Crear `supabase_migration_labs_drop_legacy.sql`:
  - `DROP TABLE laboratorios CASCADE` (verificar que no rompe RLS o triggers de otras tablas)
- Angel ejecuta el DROP manualmente
- Limpiar `LABS_REDISEÑO_NOTES.md` cerrando todos los items de deuda
- Actualizar CLAUDE.md cerrando los items de deuda registrados

**Archivos PERMITIDOS**:
- `src/hooks/useUltimasMedicionesParaDashboard.ts` (crear)
- Componentes de `/expediente/[id]/estado/` (modificar consumidores)
- `src/hooks/useLaboratoriosNormalizados.ts` (eliminar)
- Página vieja de `/laboratorios` con todas sus subrutas (eliminar)
- `supabase_migration_labs_drop_legacy.sql` (crear)
- `CLAUDE.md` y `LABS_REDISEÑO_NOTES.md` (actualizar)

**Archivos PROHIBIDOS**:
- Componentes nuevos de `/labs/` (ya construidos en sub-fases 2-6)
- Modal global de Documentos (ya extendido en sub-fase 6)
- Cualquier feature nueva — esta sub-fase es solo refactor + cleanup

**Criterios de cierre**:
- `/estado` funciona idénticamente con datos del nuevo modelo
- `grep -r "useLaboratoriosNormalizados" src/` → 0 resultados
- `grep -r "from.*laboratorios.*import" src/` → 0 resultados (referencias a tabla vieja)
- Tabla `laboratorios` dropeada en Supabase (Angel manual)
- `npm run build` verde
- LABS_REDISEÑO_NOTES.md y CLAUDE.md actualizados
- Validación visual end-to-end de `/estado` y de `/laboratorios` con paciente real

**Commit sugerido**: `chore(labs): migrar /estado al nuevo modelo y eliminar sistema legacy`

**Riesgos**:
- Si `useLaboratoriosNormalizados` tiene consumidores fuera de `/estado` que se nos escaparon en sub-fase 0, el build fallará. Resolver inline.
- DROP TABLE es irreversible. Backup mental: la data de `laboratorios` es de prueba (confirmado por Angel al inicio), borrable.
- Si las RLS policies de la tabla vieja referencian `laboratorios` desde otra tabla, CASCADE las elimina también — verificar.
