...
1. **Paso 1 — Crear el RPC v2.** Aplicar el script que crea `crear_paciente_con_medico_v2` (SQL al Dashboard). Incluye `NOTIFY pgrst, 'reload schema'`. Verificar post-flight + que la función está en el cache de PostgREST con una llamada de prueba controlada (smoke test con sesión simulada).
   - **Ventana Paso 1 → Paso 2 (inocua):** durante el lapso entre crear `v2` y desplegar el código del Paso 2, el código viejo sigue desplegado y sigue llamando a la función original `crear_paciente_con_medico` (que aún existe y funciona con su detección de duplicados local actual). La función `crear_paciente_con_medico_v2` recién creada no es llamada por nadie hasta el deploy del Paso 2. Por tanto NO hay estado roto observable en esa ventana.
...
4. **Paso 4 — Eliminar la función vieja.** Una vez el código nuevo está estable y verificado en producción, `DROP FUNCTION crear_paciente_con_medico(jsonb, uuid)`. (SQL al Dashboard.) La **verificación previa** consiste en confirmar que NINGÚN CÓDIGO DE LA APLICACIÓN la invoca — ya confirmado por la auditoría vía grep: el único llamador era `route.ts`, que en el Paso 2 pasa a apuntar a `v2`; y `sync.ts` consume el endpoint HTTP `/api/pacientes`, no la función directamente. Se entiende como "ningún código de la app la llama", no como garantía de cero invocaciones externas manuales.
   - **Nota:** la función original tiene `EXECUTE` concedido a `authenticated`, por lo que estaba expuesta como endpoint RPC vía la REST API de PostgREST. El `DROP` del Paso 4 además reduce esa superficie de ataque.
